...
- STT model setup is optional and experimental. Install or select paths for:
...
Models and runners are not bundled in the app. Use `모델 설정` -> `설치하고 사용` for SenseVoice; it downloads both the model and the Windows x64 sherpa-onnx offline package, saves `sherpa-onnx-offline.exe`, and selects it immediately. Other engines can still be wired manually from the same screen.

## Current Features

- Open audio files or drag them into the window.
- Record from the default microphone, then load it and create a 20-minute chunk ZIP.
...
- Auto-install the recommended SenseVoice model and Windows runner from the model settings window for experimental STT.
